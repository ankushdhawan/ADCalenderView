...
class ADCalendarCellView: UIView {
    var date: Date! {
        didSet {
...
class ADCalendarView : UIView {
    ...
    override func awakeFromNib() {
        ...
        cells = [ADCalendarCellView]()
        for i in 0..<42 {
            let cell = ADCalendarCellView()
            cell.tag = i
        ...
        let swipeLeftGR = UISwipeGestureRecognizer(target: self, action: #selector(ADCalendarView.advanceOneMonth))
        swipeLeftGR.direction = .left
        self.addGestureRecognizer(swipeLeftGR)

        let swipeRightGR = UISwipeGestureRecognizer(target: self, action: #selector(ADCalendarView.rewindOneMonth))
        swipeRightGR.direction = .right
        self.addGestureRecognizer(swipeRightGR)
        
        let tapGR = UITapGestureRecognizer(target: self, action: #selector(ADCalendarView.maybeGotoToday))
    ...
    @objc func didTap(_ recognizer: UITapGestureRecognizer) {
        guard let cellView = recognizer.view as? ADCalendarCellView else { return }
